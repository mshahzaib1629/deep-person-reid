...
class RetainExistingMemory():
    ...
    def __init__(self, representative_memory_directory, backup_parent_directory="./reid-data/rp-memory-backups", retain_percent = 0.5 ) -> None:
    ...
    def load_representative_images(self, memory_directory):
        image_filenames = [filename for filename in os.listdir(memory_directory) if filename.endswith('.jpg') or filename.endswith('.png')]
        images = []
    ...
    def create_backup(self):
        ...
        backup_directory = os.path.join(self.backup_parent_directory, formatted_datetime)
    ...
    def retain_existing_memory(self ):
        ...
        current_memory_grouped_images = self.load_representative_images(backup_directory)
        label_map = {}
        ...
        for g_idx, image_group in enumerate(current_memory_grouped_images):
            ...
            print(f"Retained {num_selected} images of {self.get_image_label_from_json(image_group[0]['name'])} out of {len(image_group)}")
